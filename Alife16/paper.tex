...
\author{Juan-J.~Merelo*$^1$, Mario Garc\'ia-Valdez$^2$\\
\mbox{}\\
...
programming interface. An enhanced implementation, {\sf NodIO-W$^2$} is also
presented, this version leverages the HTML 5 Web Workers specification
to execute several clients per browser in the background, so that the the number of
generations per user could be improved. The architecture, along with some initial
...
updating the content that is displayed \citep{flanagan2006javascript}.
...
single thread. The Web Workers specification \citep{hickson2012web} defines an
...
\citep{hickson2012web} workers are expected to be long-lived, they have a high
...
\begin{figure}[!htb]
\centering
\includegraphics[width=3in]{Algorithm.pdf}
...
al. \citep{chambers2005measurement}. It is important to note that, in
...
There are no intermediate byte codes or an interpreter \citep{Gray:2009:GCM:1610564.1610565}.
...
Large-Scale Global Optimization \citep{tang2007benchmark} is used in this section. 
...
browsers and desktop \citep{tilkov2010node}. These tools included
...
\begin{figure}[!htb]
\centering
\includegraphics[width=3in]{rastrigin-fitness.png}
