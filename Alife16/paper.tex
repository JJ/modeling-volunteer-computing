...
Figure \nameref{fig:system} describes the general system architecture and
...
outlined in the State of the Art (Section \nameref{sec:soa}):
\begin{itemize}
...
are shown in Figure \nameref{fig:baseline}. The baseline hardware system has these
characteristics: 
\begin{verbatim}
...
Table \nameref{tab:runs}. In general, every experiment took several
...
\nameref{tab:summary:os}, which shows the median number of IPs
...
shown in Figure \nameref{fig:duration}, the {\em addition} of more computers does
...
duration, shown in Figure \nameref{fig:zipf:os} and which roughly follows
...
in Figure \nameref{fig:puts:os}. These results show a Zipf-like behavior,
so we have fitted it to the Generalized Extreme Value distribution,
with the resulting parameters shown in Table \nameref{tab:puts:os}.
%
\begin{table*}
...
value. In this case, Table \nameref{tab:puts:os} shows $\xi$ values
...
Figure \nameref{fig:puts:os}, indicating that the fit is good enough. The
