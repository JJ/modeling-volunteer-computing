...
platforms to avoid single points of failure (the server) but, since they
need a certain amount of infrastructure installed in the client, the
threshold to join them is much lower; but once the infrastructure of P2P systems is in place
the gathering of new users becomes 
...
\citep{nogueras2015studying}  or included in a more general 
study of the performance of the EA itself
\citep{DBLP:journals/gpem/LaredoBGVAGF14}. This performance cannot be
measured without first understanding the dynamics of this kinds of systems. Initial
...
the next. 
%However, previous experiments have proven that the influence
