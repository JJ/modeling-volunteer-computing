...
last case, is labeled {\tt cachecrash}, clients rebooted if the cache
...
encourage user engagement and also to increase the involvement
...
detect changes in the cache implying a new run, this way overlaps are almost
...
should remark that some clients will still stop contributing if they
...
strategy 30\% of users do not. This might be due to the fact that {\tt
  reboot} is done only when the cache size is less than one, indicating
that this strategy is only partly successful in avoiding overlaps.  

In fact, Table \ref{tab:correlation}, shows
...
compression entropy of every user contribution for the whole %please check 
...
efficiency and eventually find out on what this efficiency depends
on. The baseline configuration that we had used in other papers 
\cite{jj-ppsn98anonymous} included
...
and this overlap was not so important.
