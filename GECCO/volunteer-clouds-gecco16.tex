...
but it remains a challenge to harness the browser's capabilities and
...
experiments with a volunteer computing framework called {\sf NodIO}
that is easily deployable in using free cloud resources. We use 
%Pablo: "deployable in using free cloud resources" is correct?
different browser configurations to find out which one is 
...
to use the computer capabilities fully, and there is some common model
to all sets of users, following a common statistical distribution. As
a pragmatic result, establishing trust with the user by open sourcing
not only the software, but also the data, is essential to have a user %Pablo: remember, is "yiuser" :P
...
% Not sure. Maybe not. - JJ
%TO DO: Add reference
There is, then, space for frameworks that do not make any of these
assumptions, such as {\sf NodIO} presented in \cite{2016arXiv160101607Manom}. {\sf NodIO} is basically 
...
JavaScript however, has become a set of standards
\cite{ECMA-262} that include the language and its components. 
...
that even
used the client's GPU \cite{duda2013gpu}. Even if, as said above, Java
...
results using a volunteer computing system that uses  % (Paloma) Change to "which" in order to not repeating "that" all the time
computational resources that are as dissimilar as smartphones and
powerful laptops 
or workstation computers in a research center. This will be described next.
...
been described in \cite{2016arXiv160101607Manom}. They prove that a bigger population
...
results were published here \cite{DBLP:conf/gecco/GuervosG15anom}. For the
...
previously obtained \cite{DBLP:conf/gecco/GuervosG15anom} which found 6 to be
