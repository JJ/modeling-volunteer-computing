...
The baseline, called {\em base} in the Table, uses the simple
...
should remark that still the clients will stop contributing if they
...
effective PUTs will have a positive impact in the algorithm.
...
fact, as can be seen in the last column, the median number of
...
contributing to the common cache.  
...
so that it became as effective as possible, and able to use
computation by volunteers in the best possible way, or at least better
...
This future holds many challenges which once again will go in two
