...
for ( i in these.experiments ) {
    this.data <- read.csv(paste0("../data/2016-PPSN/ips-reboots-cache=32-",i,".csv"))
    this.len <- length(this.data$PUTs)
    seq <- 1:this.len
    ips.data <- rbind(ips.data,
                      data.frame(Experiment=rep(i,length(this.data$PUTs)),
                                 idu=seq,
                                 idx=seq/this.len,
                                 PUTs=as.integer(this.data$PUTs),
                                 reboots=as.integer(this.data$reboots))
                       )
    
    ips.aggregates <- rbind( ips.aggregates,
                            data.frame(Experiment=i,
                                       PUTs = median(this.data$PUTs),
                                       reboots = median(this.data$reboots),
                                       avgPUTs = mean(this.data$PUTs),
                                       avgreboots = mean(this.data$reboots),
                                       totalIPs =this.len )
                            )
}
@
...
The baseline, called {\em base} in the 
