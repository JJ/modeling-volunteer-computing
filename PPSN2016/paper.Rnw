...
this paper, we analyse a volunteer evolutionary computing system called
...
%Pablo: añadiría un ", such as...," con un ejemplo entre "algorithm" y "contribute"
...
simplified standard interface for interacting with servers.
% These are "things", not "facts". As facts would be... "every browser runs a Javascript virtual machine and that there is a simplified standard interface for interacting with servers (?)
Thus
...
all available resources. This design will be done incrementally by
...
contributions, independently of how many they are, and is able to find
the solution to the problem in less time and with less contributions.
...
\item A REST (representational state transfer) server that responds to
  ...
  that is reset when the solution is found. This cache has a finite
  size that erases the oldest chromosomes once it has reached its % Also "it has been filled to its"
  capacity.  
...
this function runs in the
browser, but the server needs to know when the solution has been
found, so a termination condition is also included in the server, as a configuration
...
\caption{Ranked and normalized number of contributions (PUTs) per IP,
  plotted in a log $y$ scale.\label{fig:zipf}}
\label{fig:zipf}
\end{figure}
%
...
previous work and having a very similar appearance for all 5 sets, it
...
A summary of the experiments and  its results is shown in Table
...
the clients. The two last rows have a low number of evaluations,
...
time to find the solution {\em using the same number of
  evaluations}. All variables are difficult to control in this
...
contributions per user is {\em much lower} in the last row, which
indicates that {\tt cachereboot} needs less contributions by users to
...
strategy 30\% of users do not. This might be due to the fact that {\tt
  reboot} only does so when cache size is less than one. With very
fast clients there might be some clients that do not detect that
change and might, in fact continue with their former run, thus not
contributing to the current common cache.  

In fact, as can be seen in Table \ref{tab:correlations}, which shows
the correlation between the number of IPs from one minute to the next,
the behavior of all experiments but the last one are similar, with a
very high correlation indicating that the number of volunteers does
not change from one minute to the next. However, there is a sharp drop
for the {\tt cachereboot} experiment, which indicates a bigger
dynamism, with individuals joining and leaving the experiment all the
time. This might be circumstancial, but it also reflects the fact
that the solution is found in less time and that users need less
contributions to find it personally. 

%---------------------------------------------------------------
\section{Conclusion}
\label{sec:conclusion}

Our main intention in this paper was to focus on the algorithm itself
so that it could became as effective as possible, and would be able to use
computations by volunteers in the best possible way, or at least better
than the initial configuration. This initial configuration included
...
running heterogenous algorithms in the same way that was done in
\cite{DBLP:journals/grid/ValdezTGVO15}. Diversity, as in any other
evolutionary algorith, seems to be the key, but there are many ways of
approaching it. 
...
2015 UGR) . We would also like to thank the anonymous reviewers of
previous versions of this paper who have really helped us to improve
