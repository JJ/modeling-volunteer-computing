...
%% \footnote{It is not guaranteed to be running,
...
next, presents a similar  behavior for all experiments but the last one: a
very high correlation indicating that the number of volunteers changes
slowly. The {\tt cachereboot} experiment correlation indicates a bigger
dynamism, with individuals joining and leaving the experiment all the
time. This might be circumstancial, but it also informs the fact
that the solution is found in less time and that users need less
contributions to find it. 
