...
experiment.data <- <- data.frame(Experiment=character(),
                                 ...
                                 nooverlap = character(),
                                 reboot = character(),
                                 cachereboot = character(),
                                 milliseconds=character(),                           
                                 IPs=character(), 
                                 actualIPs=character(),
                                 PUTs=character(),
                                 actualPUTs=character())
these.experiments <- c("nooverlap-1","nooverlap-newgraph", "nooverlap-newgraph-2","nooverlap-newgraph-3","nooverlap-newgraph-reboot")
...
for ( i in these.experiments ) {
    ...
    these.experiments <- rbind( these.experiments,
                               data.frame( Experiment=rep(i, length(this.data$IPs)),
                                          newgraph = rep(newgraph, length(this.data$IPs)),
                                          nooverlap = rep(nooverlap, length(this.data$IPs)),
                                          reboot = rep(reboot, length(this.data$IPs)),
                                          cachereboot = rep(cachereboot, length(this.data$IPs)),
                                          milliseconds=this.data$milliseconds,
                                          IPs=this.data$IPs,
                                          actualIPs=this.data$actualIPs,
                                          PUTs=this.data$PUTs,
                                          actualPUTs=this.data$actualPUTs))
}
...
evolutionary computation experiments and try to find out which are
those patterns and what makes an experiment successful or not,
including the feedback loop that is created between the users and the
algorithm itself. In this paper we will examine what kind of patterns
we can expect and what influence those patterns have on the
...
called NodIO \cite{2016arXiv160101607M}. NodIO provides server
...
\cite{vespignani2009predicting}. In this paper, we are going to focus
...
\cite{Desell:2008:AHG:1389095.1389273,duda2013distributed,DBLP:journals/corr/abs-0801-1210}. Many other researchers have
...
\cite{DBLP:journals/gpem/LaredoBGVAGF14}. This performance cannot be
...
computing by Laredo et al. \cite{churn08,laredo2008rcp}. A similar
study was performed by Martínez et al. on the Capataz system
\cite{martinez2015capataz}; however, in this case the number of
...
\cite{javadi2009mining,agajaj} found out that the Weibull, log-normal and
Gamma distribution 
...
users. Apol{\'o}nia et al. \cite{apolonia2012enhancing} used the 
...
\cite{2015arXiv151101088M} that shows that performance of JavaScript
...
function \cite{Ackley1987} has been used. Depending on the
