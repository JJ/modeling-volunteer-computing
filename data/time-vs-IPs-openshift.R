...
ggplot(data=ips.time, aes(x=reorder(i,-milliseconds),y=milliseconds,color=IPs)) +geom_point()+scale_y_log10() + scale_color_gradient(low='blue', high='yellow')+theme(axis.ticks = element_blank(), axis.text.x = element_blank(),axis.text.x=element_text("Ordered experiments"))
