...
\url{http://nodeo-i.herokuapp.com}) where presented, with a short
...
a difficult problem for evolutioanry algorithms. In
...
``browserification''. The algorithm runs a canonican evolutionary
...
and random individual; this algorithm can be also run locally, the
algorithm does not stop if there is no answer from the server. As a
summary, the only thing needed to run an evolutionary algorithm in the browser is to add {\em migration} functions to the original one. 
...
code also. The main changes to the code from the  the previous paper
\cite{arxiv:modeling} is, besides the inclusion of logging for a
better experiment analyis, the function that checks for the algorithm
...
previous version. %FERGU: es esta la única innovación al paper de Arxiv? A lo mejor no deberíamos mencionarlo ni usarlo como base

All in all, it amounts to a few hundred lines of code
including all languages (client and server JavaScript, HTML and CSS). This code can
be deployed, in principle, in any server that is able to run node.js. 
...
or no modification, to other similar PaaS, maybe with a bit of adaptation. Any of them are
...
modification, to any PaaS is not difficult, but you have to consider
the different capabilities of each one.\begin{itemize}
\item Heroku does not have a permanent filesystem, so if you want to
  ...
  started. It is only guaranteed to last while the application is
  running. 
\item In order to scale properly, all requests in Openshift are routed
  through a proxy. This means that the requests all have, as remote
  address, this IP. Other ways of obtaining the requests must be found
  out. On the other hand, it does have a permanent directory for data.
\end{itemize}

Every PaaS has its own way of defining environment variables, which is the way
used for passing private information such as passwords to the
...
default Ajax mode, that does not allow cross-site requests. However, %FERGU: no todo el mundo sabe de Ajax y lo de cross-site, explícalo mejor
...
In this case, every domain holds %FERGU: si es referencia a lo de parasitar de arriba ponlo en el mismo párrafo, que parece que te refieres a lo "parasitar". Mejor "In this study"?
...
repository \url{https://github.com/JJ/splash-volunteer}, and they 
...
%pondría las barras de Ips distintas (del 1 al 7 y del 1 al 10).
%jj: no te entiendo, pero los datos están por ahí - JJ
...
%FERGU: dÃ­ que la persona que lo anunciÃ³ en twitter tiene X seguidores
% OK, con los nuevos experimentos lo haré. De hecho, deberíamos hacer
% experimentos anunciándolo tú y anunciándolo yo. - JJ
\begin{figure*}[htb]
        \centering
