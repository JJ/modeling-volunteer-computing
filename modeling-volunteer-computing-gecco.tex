...
conferences\footnote{Names withheld for the double blind review}. During the %FERGU: TODO JJ, no te olvides de decirme esto. Es la IDC 2014?
...
module and has three REST \cite{Castillo12REST} %FERGU: cito a este paper que es un capítulo de libro en vez de Arxiv
...
repository \url{http:\\blind-review} %\url{https://github.com/JJ/idc-keynote}. FERGU: JJ, es este el repo correcto, no?
These results
...
work and found the solution, passed. %FERGU: que quiere decir esta primera frase?
In fact, the first version of the
algorithm (until January 2015) used too much memory on the browser,
which crashed after a while running. A total of 7 experiments were %el "were" es para los experimento o para el "total" A total was?
...
\end{figure*}
%FERGU: uhm, yo en vez de poenr 2 veces 7 ips distintas, simplemente pondr�a las barras de Ips distintas (del 1 al 7 y del 1 al 10).
...
investigate further the speedups achieved and if these are %FERGU: speedup va junto
...
evaluations and the associated evolutionary geneticry. The vast %FERGU: geneticry?
