...
purposes of this experiment OpenShift (\url{http://openshift.com}), a Platform as a Service with a
free tier, has been chosen. It could be also be deployed, with small
or no modification, to Heroku or other similar PaaS. Any of them are
...
the logs: the log file (if the PaaS allows writing to the filesystem,
which is not the case in Heroku) or to online logging services such as
...
free) will have to be sought in the future. Other online logging
services have a free tier that can be used for ever; however, its
support and facilities are not so good. Choosing a good logging
service for experiments in open science is an open issue but it is
