...
evolutionary compuation experiments \cite{Merelo2014,jj:idc:lowcost}.
...
In this case, every domain holds
...
easiness and support from the {\tt winston} logging library; however,
it is only free for a month, so a more sustainable alternative (read:
free) will have to be sought in the future, after this paper is finished. Other online logging
services have a free tier that can be used for ever; however, its
support and graphing facilities were not so complete. Choosing a good logging
service for experiments in open science is, for us, an open issue but it is
outside the scope of this paper. It will have to be left for future
work. 
% Include loggly graphs here.
...
repository \url{https://github.com/JJ/splash-volunteer}. 
