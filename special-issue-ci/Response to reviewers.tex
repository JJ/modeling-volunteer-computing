...
have published these tests as an open access technical report.  % quizás indicar la URL aquí
...
\end{quote}
I fact 3.46 is the average for all 50 runs. We have enphasized the
word average to make that clearer.
...
section. Besides, we have added a more difficult probmem, the shifted
Rastrigin function, to make measures in a different scenario (one in
which the problem is not solved in a few seconds). 
...
it shadows the time in the case there is a single ocassion. In the
...
problem is two or five times that achiever for JavaScript, this
...
added as non-volunteer users, since the heterogeinity and simple API
...
We have extended the experiments section with a new one, based on the
hard Rastrigint problem. 
