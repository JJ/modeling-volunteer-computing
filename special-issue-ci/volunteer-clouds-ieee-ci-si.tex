...
be some issues with the performance of the language itself. This
 is
...
Java or C might be, indeed, faster, but the fact % (Paloma) Too many "fact" words. There was another one in the previous sentence.
 that a single-user
performance is higher is more than offset. This is because the number
...
\end{enumerate}
  ...
  works the application programming interface (API) can be used, % (Paloma) I thought that a client implements an API, or uses an API, or even works with an API. But never heard of a client which "works" an API. Is this correct?
...
\item {\em Adaptiveness} is achieved simply through the autonomous
  ...
  the scalability of the system. % (Paloma) Is it ok if we talk about scalability when we were talking about adaptiveness? Scalability was also described in the first point.
