...
reasonable performance. But hierarchy, this is, the capability of preferentially using resources in the same organization, is a feature that could be either disregarded or included within adaptivity, which leaves us with eight desirable properties.
...
\cite{baratloo1999charlotte}, but it is interesting to note that, even in the early stages of the web, the possibility of 
...
since it yields % (Paloma) Use synonyms, like "yield" or "permit". Repetitive use of "allow" is... repetitive.
...
volunteer computing, with regard to
number of users and computation time spent by every one 
in browser-based volunteer computing experiments, have
...
As far as we know, this paper presents one of the few experiments which % (Paloma) Change to "which" in order to not repeating "that" all the time
...
Distribution & Location $\mu$ & Scale $\sigma$ & Shape $\xi$ \\
\hline
GEV & 35.759 $\pm$ 0.83719  &  343.336  $\pm$  & 9.877   $\pm$
0.04746 \\
