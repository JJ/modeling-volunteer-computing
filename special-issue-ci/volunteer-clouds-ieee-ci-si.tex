...
\begin{abstract}
Volunteer computing is extensively used in science and industry, but it
still has a long way to go in order to become a mainstream
technology in the same way the cloud technologies in general are. %I have changed "have" to "are"
The lack of general purpose and open source tools with built-in analytics 
...
evolutionary algorithms, entirely written in JavaScript, which needs
minimal server support to work and that is able to support persistent,
...
presented; this version leverages the HTML5 Web Workers specification
...
the market, usually as open source systems \cite{Parejo12Survey}. 
...
sources, client and server, for the application, are openly released. 
 We also release as open access the performed experiments \cite{DBLP:journals/corr/GuervosG15} 
