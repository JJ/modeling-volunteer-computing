...
\item A client that includes the evolutionary algorithm as
  ...
  using Web Workers. 
  % Repeated
  %which run in the background even if the webpage or
...
asynchronous Web applications. Scripts are embedded in HTML pages and change
the state of with the Document Object Model (DOM) of the page, by sending or
retrieveing data asynchronously from a server. For these purpose JavaScript has
been traditionally implemented as an interpreted language not designed for the
development of high performance systems. Current JavaScript virtual machines (VMs)
are closing the gap, for instance the Google V8 is an engine specifically designed
for fast execution of large JavaScript applications. Also V8 compiles JavaScript
source code directly into machine code when it is first executed. 
There are no intermediate byte codes or interpreter.
...
To further evaluate how a current JavaScript implemantation can be used by a
computer scientist for the development of complex optimization problems,
a benchmark function, which was provided by CEC2010 Special Session on
Large-Scale Global Optimization [] as Function 15 is used in this section. 
The function is described next.


The basic Rastrigins function is separable[], and is defined as follows;
\begin{equation}
F_{rastrigin}(x)=\sum\limits_{i=1}^\frac{D} [x_{i}^{2}-10\cos(2\pi x_i)+10  ] 
\end{equation}
...
\begin{equation}
F_{rot_rastrigin}(x)=F_{rastrigin}(\textbf{z}), \textbf{z}= \textbf{x} * \textbf{M}
\end{equation}
Where \textbf{M}_{is} a $D×D$ orthogonal matrix. 

The benchmark  emulates real-world optimization problems that most likely will consist 
...
of separability:
\begin{equation}
F_{15}(x)=\sum\limits_{k=1}^\frac{D}{m}F_{rot_rastrigin}[\textbf{z}(F_{(k-1)*m+1}:P_{k*m})] 
\end{equation}
For the benchmark $D = 1000$, Group size $m = 50$, $\textbf{x} = (x_1, x_2, · · · , x_{D})$ is the
candidate solution, $\textbf{o} = (o_1, o_2, · · · , o_{D})$ is the shifted global optimum,
$\textbf{z} = \textbf{x} − \textbf{o}$ is the shifted candidate solution and
finally $\textbf{P}$ is a random permutation of ${1, 2, · · · ,D}$.
%---------------------------------------------------------------
\section{Conclusion}
