...
still has a long way to go if it is going to become a mainstream technology in
the same way the cloud technologies in general have. The lack of general
...
application, in which clients can run in a browser. 
Therefore, they can embedded any web-enabled device, including many over which 
...
that the source code can be directly examined by volunteers, because is not
...
safety, which introduce a {\em social} aspect to the design of a
...
powerful laptops 
or workstation computers in a research center. The methodology used to
...
algorithm, data types and structures employed in large scale optimization problems. 
The function was implemented in both Matlab and Java languages as part of  the
Test Suite of the companion competition of the Special Session. To give competitors an idea of the
...
a guide, the function described above was implemented to compare the performence of JavaScript against
both implementations, again using as a test, the time required for doing a 10,000 FEs. 
...
Java Randomizer library for the implementation of the 
randomization of shift vectors and matrices used for by the function.
The library included the generation of pseudorandom Gaussians of type
...
in the implementation of {\tt Math.random()} between engines and their results are non-deterministic. 
The library {\tt random-js}, implements a Mersenne Twister algorithm producing 
consistent results across all JavaScript VMs assuming the same seed.
...
the runtime is needed. In JavaScript the {\tt Date class} is often used 
to measure execution time, but has the limitaoun of having a maximum resolution given in miliseconds.
In order to measure the runtime intervals with a higher precission two functions
where used: for Node.js we used the native {\tt process.hrtime()} function
which returns a high-resolution measure in a {\tt [seconds, nanoseconds]} array.
For testing in the browser, the {\tt Performance.now()} function was used, this
function uses floating-point numbers with up to microsecond precision. 
...
\item {\em Tools} An important consideration when chosing certain language to 
to write non-trivial programs is the availability of development tools. When developing
the experiments, we used a comprehensive set of developer tools available in both
browsers and desktop. These tools included package managers, debbugers, logging, 
...
limited precision of 64 bits that partialy implement the functionality of
the {\tt StrictMath} library used by the Java implemantation. If more precision
is needed developers could use {\tt math.js}, an extensive math library with support
for matrices, big and complex numbers.
\end{itemize}

We found that the JavaScript ecosystem is mature enough for developing non-trivial 
code, but developers have to consider the differences betwwen VMs implementations
and also use certain libraries in order to increase the precision and repeatability of
algorithms. As for the performance of the JavaScript implementaion against Java and Matlab
we conducted first the basic test described earlier in a 3.7 GHz Quad-Core Intel Xeon E5 processor running OS X 10.10.5, 
...
and Firefox version 41.0.2. The performance of the time required for 10,000 FEs is presented in Figure \ref{fig:f15_times}.
In our test the Matlab implementation had the best performance with an average of 935ms, followed by Java with
991ms, JavaScript took 32\% more time than Java with 1240ms in Chrome using a Web Worker and 1,270ms using Node.js. 
The results also show that there is not much difference between the Node.Js implementation an that of a
browser. There is also not much difference between running the code in the main thread or as workers, but
when running the experiment in Firefox a dialog alerted of the script taking to long, showing that this
kind of processing is not intended for the main thread. Also there is not much difference when running two
Web Workers in parallel taking 1279ms each. This experiment shows that JavaScript is viable 
language for developing complex mathematical functions for running in the browser. 
\begin{figure}[!htb]
\centering
