...
% data - JJ
find out what kind of patterns the user behaviour follow and try to fit them to some statistical distribution.
\end{abstract}
...
hardware. This was aggravated by the increasing maintance costs and extended downtime resulting from the continuous failures of existing clusters.
...
\item A REST server, that is, a server that includes several remote %remote requests suena raro, propongo algo como: remote procedures o web interfaces   
  ...
  to performance and state of the experiments. It also peforms logging
...
%FERGU: poner algo tras el handling: handling it?
%handing - JJ
...
\begin{tabular}{l|r}
\hline
Date & Number of experiments & Different IPs \\
\hline
...
\caption{Summary of time per run, number of IPS and number of {\tt
    PUT}s per IP in the initial runs. \label{tab:runs}}
\begin{center}
\begin{tabular}{l|rrr}
\hline
Date & Median IPs & Median time (s) & Median #{\tt PUT}s  \\
\hline
4/4& & 5 & 2040 & 18\\
April 24th 4/24 &  5 & 732 & 11\\
July 31th 7/31 & 5 & 260 & 23 \\
\hline
\end{tabular}
\end{table}
%
