...
There is, then, space for systems that do not have any of those %[Mario] these assumptions? 
...
page where the client is embedded, including possibly many over which%[Mario]Quitar el possibly?   
...
hardware and continuous hardware failures of existing
clusters. 
%[Mario] Otra opción: Some time ago, our group faced the problem of diminishing funds 
%for buying new hardware. This was aggravated by the increasing maintance costs 
%resulting from the continuous failures of existing clusters.
Considering this, we leveraged our experience of working
with web applications, including JavaScript, since their inception and other volunteer and
unconventional distributed evolutionary computing systems, and we 
decided to designd and relase a new free framework that would allow anyone to
...
in most Platforms as a Service (PaaS) as many support the Node.js framework \cite{wood13:nodejs:paas}. Also many web services %FERGU: he validado esta frase con un footnote, pero si quieres lo paso a referencias
...
when writing code and leverages %FERGU: párrafo en pasado "was used" y
                                %presente "saves" 
% done - JJ
...
% Visto :-) Gracias- JJ
There were some initial attempts to simply dodge the issue by making
...
% [Mario] Nota: se menciona también en el segundo párrafo de la Introducción.
...
\item A REST server, that is, a server that includes several remote %remote requests suena raro, propongo algo como: remote procedures o web interfaces   
  ...
  %The information is of two kinds: Problem Based y ¿Cual otra? ¿Logging? No se entiende bien. ¿Podría ser problem domain?   
...
Figure \ref{fig:system} describes the general system architecture.
%
\begin{figure}[!t]
\centering
\includegraphics[width=4in]{img/system.pdf}
\caption{Description of the proposed system. Clients execute a JS EA in the browser, which, every 100 generations, sends the best individual and receives a random one from the server. } 
\label{fig:system}
\end{figure}
...
function and handling at the creation of the algorithm object, called %FERGU: poner algo tras el handling: handling it?
...
\item The system is {\em heterogeneous} since it does not need any
  ...
  visiting the page, even from mobile devices, can load the algorithm. %Fergu: puesto lo de mobile devices para venderlo aún mejor
...
destktop, their interest will be purely academic. We will try and
