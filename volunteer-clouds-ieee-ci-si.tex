...
state of the art in web-based distributed computation systems along
with attempts to predict and model its behavior. 
...
and DropBox~\cite{DBLP:conf/cec/ArenasGMCRL11}. The use of JavaScript to implement server side components, 
gives software developers certain advantages in the cloud paradigm. JavaScript is a first-class citizen
in most Platforms as a Service (PaaS) as many support the nodejs framework. Also many web services
...
%Ayuda con la mini conclusion
%In this work JavaScript was used in the full stack, some percieved advantages are discussed?
%The proposed software has the advantage of full stack JavaScript development..
...
As far as we know, this paper presents the only experiment that uses computational %FERGU: yo no estaría tan seguro de esto
...
% Ok, lo escribo. - Mario
% Voy escribiendo algo de HTML5 y Web Workers?, no necesariemente para esta sección. 
