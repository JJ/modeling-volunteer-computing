...
\author{Juan-J.~Merelo*$^1$, Mario Garcia-Valdez$^2$, Pedro A. Castillo$^1$, Pablo Garc\'ia-S\'anchez$^1$
...
run on a  % [Pedro] Both the server and the client in browsers?
          % different browsers as I understand?  No, server in
...
Some time ago, our group faced  the problem of diminishing funds for buying new     % [Pedro] This sentence is so so long. I would split it in two sentences.
hardware and continuous hardware failures of existing
clusters. Considering this, we leveraged our experience of working
...
In this sense, in this paper we propose {\sf NodIO}, a PaaS based 
...
\caption{Comparison of time to solution for the baseline system, using a {\tt
    node.js} client, for different population sizes. They only compare
...
with success rate upgraded to 100\%. Results for these two experiments
